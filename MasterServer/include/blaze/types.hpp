...
enum class StatsCommand : uint16_t {
    getStatGroup               = 0x04,
    getLeaderboardGroup        = 0x0A,
    getLeaderboard             = 0x0C,
    getCenteredLeaderboard     = 0x0D,
    getFilteredLeaderboard     = 0x0E,
    getStats                   = 0x10,
    getLeaderboardEntityCount  = 0x12,
};

enum class GameReportingCommand : uint16_t {
    submitGameReport  = 0x02,
    startGameSession  = 0x64,   // game-custom (frostbite_multiplayer)
};
...
struct ServerConfig {
    ...
    // QoS UDP probe responder (runs in parallel with the Blaze server). The client is
    // told this port + qos ip over HTTP (/qos/qos) by the Go blazeServer; it then UDP-
    // probes here and the reply reflects its public ip:port back for NAT discovery.
    // Firewall ports are qos_probe_port+1 and +2.
    std::string qos_host = "0.0.0.0"; // bind address for the UDP sockets
    uint16_t    qos_probe_port = 42231;
