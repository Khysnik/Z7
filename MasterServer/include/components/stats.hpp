...
private:
    std::unique_ptr<blaze::Packet> handleGetStatGroup(const blaze::Packet& request, std::shared_ptr<ClientConnection> client);
    std::unique_ptr<blaze::Packet> handleGetStats(const blaze::Packet& request, std::shared_ptr<ClientConnection> client);
