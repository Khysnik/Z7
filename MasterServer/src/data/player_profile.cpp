...
void PlayerProfile::applyGameReport(const std::unordered_map<std::string, double>& report) {
    ...
    for (const auto& [name, value] : report) {
        // DIAG (xp/coin balloon): flag any implausibly large report value. A garbled
        // parse (wrong 05-01-0A map / misread float) surfaces here as a huge number,
        // and an "Increment" stat then accumulates it. Reproduce the bug and grep this.
        if (std::isfinite(value) && value < kSentinel && std::abs(value) > 1.0e7)
            LOG_WARN("[PlayerProfile] LARGE report stat {} = {} (possible xp/coin balloon)", name, value);
        auto it = m_index.find(name);
        if (it == m_index.end()) continue;
        std::string& cur = m_stats[it->second].second;
        // Keep fractional precision: a stat is float if it's *stored* with a decimal OR
        // the incoming value has a fractional part. Without the latter, a GR best-time
        // seeded/saved as an integer string (e.g. "1000000") loses its decimal and every
        // time gets llround()'ed to the nearest second on the leaderboard.
...
void PlayerProfile::setStat(const std::string& name, double value, bool asFloat) {
    auto it = m_index.find(name);
    if (it == m_index.end()) return;
    m_stats[it->second].second = formatValue(value, asFloat);
}
