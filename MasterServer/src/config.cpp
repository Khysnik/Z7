...
// Dedicated gameserver matchmaking dials the client at. IP is a big-endian
// uint32 (a<<24|b<<16|c<<8|d): 2130706433 = 127.0.0.1. Default port matches the
// Go dedicatedServer (:10018). The Go server must be configured with the same
// GameUuid/PlayerUuid used in the dedicated game setup so the tunnel key matches.
const std::int64_t  gameServerIp   = 2130706433;   // 127.0.0.1
const std::int64_t  gameServerPort = 10018;
...
bool                onlineMode                = false;   // set true by -online
