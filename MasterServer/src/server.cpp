...
bool Server::init(const blaze::ServerConfig& config) {
    ...
    // QoS UDP responder, in parallel on the same io_context.
    if (config.qos_enabled)
        m_qosServer = std::make_unique<network::QosUdpServer>(m_io_context, config.qos_host, config.qos_probe_port);
...
void Server::start() {
    ...
    m_blazeServer->start();
    if (m_qosServer) m_qosServer->start();
