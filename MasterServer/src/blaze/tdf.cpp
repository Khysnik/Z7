...
// Blaze encodes Float on the wire as a 4-byte BIG-endian IEEE-754. x86 is
// little-endian, so a raw memcpy mangles the value (e.g. 34.0 = 0x42080000 BE
// read verbatim becomes a ~0 denormal, which then loses TSHOOT leaderboard
// scores under High aggregation). Swap bytes on both encode and decode.
static float byteswapFloat(float v) {
    uint32_t bits;
...
void TdfEncoder::encodeList(const std::string& tag, TdfType elementType, const TdfList& value) {
    ...
    for (const auto& elem : value) {
        ...
        switch (elementType) {
            ...
            case TdfType::Float: {
                float f = byteswapFloat(std::get<float>(elem->value));
    ...
    for (const auto& [k, v] : map.data) {
        ...
        if (v) {
            switch (valueType) {
                ...
                case TdfType::Float: {
                    float f = byteswapFloat(std::get<float>(v->value));
                    writeBytes(&f, sizeof(float));
                    break;
                }
                default:
...
void TdfEncoder::encodeFloat(const std::string& tag, float value) {
    ...
    float be = byteswapFloat(value);
    writeBytes(&be, sizeof(float));
}
