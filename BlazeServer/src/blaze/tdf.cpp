...
TdfList TdfDecoder::decodeList() {
    ...
    for (uint64_t i = 0; i < count; i++) {
        ...
        switch (elementType) {
            case TdfType::Integer: elem->value = decodeInteger(); break;
            ...
            case TdfType::Pair:    elem->value = decodePair();    break;
            case TdfType::Triple:  elem->value = decodeTriple();  break;
...
TdfMapWrapper TdfDecoder::decodeMap() {
    ...
    for (uint64_t i = 0; i < count; i++) {
        ...
        switch (valueType) {
            case TdfType::Integer: val->value = decodeInteger(); break;
...
static std::string renderBlazeValue(const TdfValue& v, int indent, const std::string& label) {
    ...
    switch (v.type) {
        ...
        case TdfType::Pair: {
            ...
            out = ind + label + " = (" + fmtBlazeInt(p.first) + ", " + fmtBlazeInt(p.second) + ")\n";
            break;
        }
        case TdfType::Triple: {
            ...
            snprintf(ipBuf, sizeof(ipBuf), "%u.%u.%u.%u",
                (t.ip >> 24) & 0xFF, (t.ip >> 16) & 0xFF, (t.ip >> 8) & 0xFF, t.ip & 0xFF);
            out = ind + label + " = (" + ipBuf + ":" + std::to_string(t.port) +
                  ", proto=" + std::to_string(t.protocol) + ")\n";
            break;
        }
        case TdfType::Float:
...
std::string blazePacketName(uint16_t comp, uint16_t cmd, MessageType msgType) {
    // Notification name table — keyed by (comp << 16) | notifId.
    // Names confirmed from binary analysis.
    ...
    static const std::unordered_map<uint32_t, Names> kTable = {
        ...
        // PvzGw (0x0805) — names confirmed from binary 140e62610
        ...
        // UserSessions (0x7802) — names confirmed from binary 140e62610
